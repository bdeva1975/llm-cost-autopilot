...
def _action_builder(df: pd.DataFrame, key: str) -> Action | None:
    ...
    kind = st.radio("Action", ["Route traffic", "Cap output tokens"], key=f"{key}-kind", horizontal=True)
    ...
    if kind == "Route traffic":
        ...
        return RouteAction(application=app, from_model=from_model, to_model=to_model, fraction=fraction)
    cap = st.number_input(
...
def page() -> None:
    ...
    with st.container(border=True):
        ...
        for i in range(int(n_actions)):
            ...
            if a is not None:
                actions.append(a)
        name = st.text_input("Scenario name", value=f"scenario-{len(st.session_state.whatif_results) + 1}")
    ...
    if not results:
        st.info("Build and run a scenario. Suggested first run: route 100% of app-summarizer from atlas-ultra to atlas-pro.")
        return
    ...
    fig.update_layout(height=320, margin=dict(l=0, r=0, t=10, b=0), showlegend=False, yaxis_title="USD (30d)")
